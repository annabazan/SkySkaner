...
// 17	Longitude Decimal Degrees	Floating point [-180,180]
    ...
    if (file3.is_open()) {
        string line;
            ...
            int j=0;
            int i=0;
            while(i<370){while(line[j]!=','){ cities[i]+=line[j]; j++;} j++; j++; i++;}
    ...
    if (file.is_open()) {
        ...
        for(int i=0; i<9300; i++){
            ...
            bool m = false;
            for(int q=0; q<370; q++) if(cities[q]==city) m =true;
    ...
    for(int i=0; i<WCities.size(); i++){
        int timezone = rand()%24 -12;
        if(i<WCities.size()-1)cout<<"("<<i+1<<", '"<<City[WCities[i]]<<"', "<<wyn[Country[WCities[i]]]<<", POINT("<<X[WCities[i]]<<", "<<Y[WCities[i]]<<"), "<<timezone<<"),"<<endl;
        else cout<<"("<<i+1<<", '"<<City[WCities[i]]<<"', "<<wyn[Country[WCities[i]]]<<", POINT("<<X[WCities[i]]<<", "<<Y[WCities[i]]<<"), "<<timezone<<");"<<endl;
    }
    cout<<endl;
    ...
    for(int i=0; i<40000; i++){
        ...
        int minp = rand()%60;
        if(i<39999){cout<<"("<<i+1<<", "<<airpfrom<<", "<<airpto<<", '";
